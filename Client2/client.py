...
def fillPackageList():
	"""Sucht in der Datei packages.json nach den Packagedaten, ueberfuehrt sie in Package-Objekte und fuegt sie der Liste hinzu"""
	global packageList
	try:
...
def changeVersion(newVersion, packageName):
	"""Aendert die Version des geupgradeten Packages und schreibt die neuen Daten in die Datei packages.json.
	...
	for x in range(0,len(packageList)):
		if x == len(packageList)-1:
			f.write("{\"name\":\""+packageList[x].name+"\",\"version\":\"" 
				+ packageList[x].version + "\",\"url\":\""+packageList[x].url+"\"}")
		else:
			f.write("{\"name\":\""+packageList[x].name+"\",\"version\":\"" 
				+ packageList[x].version + "\",\"url\":\""+packageList[x].url+"\"},")
	f.write("]}")
